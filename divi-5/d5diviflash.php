...
if ( ! defined( 'DIFL_MODULES_JSON_PATH' ) ) {
	define( 'DIFL_MODULES_JSON_PATH', DIFL_MAIN_PATH . 'server/modules-json/' );
}
...
require DIFL_MAIN_PATH . 'server/Modules/Modules.php';
...
function difl_module_enqueue_vb_scripts() {
	if ( et_builder_d5_enabled() && et_core_is_fb_enabled() ) {
		...
		wp_enqueue_script( 'dofl-modules-builder-bundle-script', "{$plugin_dir_url}visual-builder/scripts/bundle.js",
			array(
				'divi-module-library',
		...
		\ET\Builder\VisualBuilder\Assets\PackageBuildManager::register_package_build(
			[
				...
				'version'  => null,
				'script'   => [
					'src'  => "{$plugin_dir_url}visual-builder/scripts/content-carousel.js",
					'deps' => [
						'lodash',
						'divi-vendor-wp-hooks'
					],
...
function difl_module_enqueue_frontend_scripts() {
	$version = time();
	$plugin_dir_url = plugin_dir_url( __FILE__ );
	wp_enqueue_style( 'difl-modules-builder-bundle-style', "{$plugin_dir_url}visual-builder/styles/vb-bundle.css", array(), '1.0.0' );
	wp_enqueue_style( 'difl-modules-logo-showcase-style', "{$plugin_dir_url}visual-builder/styles/logo-showcase-style.css", array(), '1.0.0' );
	wp_enqueue_style( 'difl-modules-content-carousel-style', "{$plugin_dir_url}visual-builder/styles/content-carousel.css", array(), '1.0.0' );

	wp_enqueue_script( 'lightbox', $plugin_dir_url . 'visual-builder/scripts/lightgallery.js', array('jquery'), $version, true );
	// Swiper Load
	wp_enqueue_script( 'swiper', $plugin_dir_url . 'visual-builder/scripts/swiper.min.js', array('jquery'), $version, true );
	wp_enqueue_script( 'swiper-content-carousel', $plugin_dir_url . 'visual-builder/scripts/contentcarousel.js', array('jquery', 'swiper'), $version, true );
}
...
function tanvir_md_al_amin_footer(){
	...
	echo $temp_url.' <br> TTTTTTTTTTT '.$plugin_dir_url;
}