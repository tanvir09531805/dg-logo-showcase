...
const ButtonAppearance = ({
	...
	const TypeControl = () => {
		...
		const types = {
			fill: {
				label: 'fill',
				tooltip: __('Filled', 'divi_flash'),
				icon: 'text',
			},
			outline: {
				label: 'outline',
				tooltip: __('Outline', 'divi_flash'),
				icon: 'text',
			},
		};
	...
	const renderColors = () => {
		...
		const settings = {
			normal: {
				label: __('Normal', 'divi_flash'),
				controls: {
					background: __('Background', 'divi_flash'),
					text:
						type === 'fill' || noBorder
							? __('Text', 'divi_flash')
							: __('Text and Border', 'divi_flash'),
				},
			},
			hover: {
				label: __('Hover', 'divi_flash'),
				controls: {
					backgroundHover: __('Background', 'divi_flash'),
					textHover:
						type === 'fill' || noBorder
							? __('Text', 'divi_flash')
							: __('Text and Border', 'divi_flash'),
		...
		const shadowCommonControls = {
			color: {
				label: __('Shadow', 'divi_flash') + ' ' + __('Color', 'divi_flash'),
				type: 'color',
			},
			blur: {
				label: __('Blur', 'divi_flash'),
				type: 'range',
				min: 0,
				max: 50,
			},
			width: {
				label: __('Offset X', 'divi_flash'),
				type: 'range',
				min: -50,
				max: 50,
			},
			height: {
				label: __('Offset Y', 'divi_flash'),
				type: 'range',
				min: -50,
				max: 50,
			},
		...
		const shadowSettings = {
			normal: {
				label: __('Button', 'divi_flash') + ' ' + __('Shadow', 'divi_flash'),
						...
						>
							...
							{!noShadow && (
								<PanelRow key={`shadow_${optionType}`}>
									<Toggle
										label={
											__('Button', 'divi_flash') +
											' ' +
											__('Shadow', 'divi_flash')
										}
	...
	const borderControls = () => {
		...
		const directions = {
			top: __('Top', 'divi_flash'),
			right: __('Right', 'divi_flash'),
			bottom: __('Bottom', 'divi_flash'),
			left: __('Left', 'divi_flash'),
		};
	...
	return (
		<div className="difl-button-appearance-control">
			{label && <span className="customize-control-title">{label}</span>}
			<div
				className={`difl-white-background-control ${
					type ? 'has-type' : ''
				}`}
			>
				<span className="customize-control-title">
