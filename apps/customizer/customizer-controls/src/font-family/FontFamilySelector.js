...
const FontFamilySelector = ({
			...
			options.push(
				...
				>
					<FontPreviewLink
						...
						label={
							inheritDefault
								? __('Inherit', 'divi_flash')
								: __('Default', 'divi_flash')
						}
					/>
				</li>
			);
				...
				<div className="popover-content">
					<div className="popover-header">
						<div className="search">
							<TextControl
								placeholder={__('Search', 'divi_flash') + '...'}
								value={search}
								onChange={(e) => {
									setSearch(e);
									setLoadUntil(20);
					...
					<ul className="difl-fonts-list">
						{options.length ? (
							options
						) : (
							<li className="no-result" key="no-results">
								{__('No results.', 'divi_flash')}
							</li>
						)}
					</ul>
				</div>
	...
	return (
		<div className="difl-font-family-control">
			<span className="customize-control-title">
				{__('Font Family', 'divi_flash')}
			</span>
			<Button
			...
			>
				...
				{visible && (
					<Popover
						...
						inline={true}
					>
						{fonts ? getFontList() : __('In Progress', 'divi_flash')}
					</Popover>
				)}
			</Button>
		</div>
