...
class ImageCarousel extends Component {
    static slug = 'difl_imagecarousel';
    _isMounted = false;
    ...
    swiper_init() {
        ...
        if (props.arrow === 'on') {
            config['navigation'] = {
                nextEl: '.ic-next-'+order_number,
                prevEl: '.ic-prev-'+order_number
            }
        }
    ...
    static css(props) {
        ...
        if(props.ic_full_width === 'on') {
            additionalCss.push([{
                selector:    '%%order_class%% .ic_image_wrapper, %%order_class%% .ic_image_wrapper im',
                declaration: `align-self:${props.ic_vertical};`,
            }]);
        }
        ...
        utility.df_process_bg({
            ...
            'selector'          : '%%order_class%% .difl_imagecarouselitem .overlay_wrapper'
        });
        // button
        // button styles
        ...
        if (props.content_hover === 'on') {
            ...
            additionalCss.push([{
                ...
                declaration: `opacity: 0; transform: ${ImageCarousel.df_transform_values(anim_direction).default};`,
            }]);
            additionalCss.push([{
                selector:    '%%order_class%% .difl_imagecarouselitem:hover .content',
                declaration: `opacity: 1; transform: ${ImageCarousel.df_transform_values(anim_direction).hover};`,
            }]);
        }
        ...
        if (props.arrow === 'on') {
            ...
            additionalCss.push([{
                ...
                declaration: ImageCarousel.df_arrow_pos_styles(pos),
                'device':'desktop',
            }]);
            additionalCss.push([{
                selector:    '%%order_class%% .df_ic_arrows',
                declaration: ImageCarousel.df_arrow_pos_styles(pos_tab),
                'device':'tablet',
            }]);
            additionalCss.push([{
                selector:    '%%order_class%% .df_ic_arrows',
                declaration: ImageCarousel.df_arrow_pos_styles(pos_ph),
                'device':'phone'
            }]);
            // alignment
            additionalCss.push([{
    ...
    df_ic_arrow(order_number) {
        ...
        return this.props.arrow === 'on' ?
            <div className="df_ic_arrows">
                <div className={"swiper-button-next ic-next-" + order_number} data-icon={next_icon}></div>
                <div className={"swiper-button-prev ic-prev-" + order_number} data-icon={prev_icon}></div>
            </div> : '';
    }

    df_ic_dots(order_number) {
        return this.props.dots === 'on' ?
            <div className={"swiper-pagination ic-dots-"+order_number}></div> : '';
    }
    ...
    contentOutput(props){
        ...
        if (props['content'] === '' || props['content'].length === 0) {
            ...
            return <h2 style={notice_style} >Please <strong>Add New Image Item.</strong></h2>;
        }
    ...
    render() {
        ...
        return(<div className={"df_ic_container" + _class} ref={this.wrapper}>
            {this.state.loading === false ?
            <React.Fragment>
                <div className="df_ic_inner_wrapper">
