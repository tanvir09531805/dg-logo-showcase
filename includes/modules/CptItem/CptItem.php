...
class DIFL_CptItem extends ET_Builder_Module_Type_PostBased {
    ...
    public function init() {
        ...
        $this->display_element_dependancy = array('author', 'date', 'taxonomy', 'custom_text', 'icon', 'acf_fields', 'pod_fields', 'metabox_fields','reading_time');
    }
        ...
        return array(
            'general'   => array(
                'toggles'      => array(
                    ...
                    'pod_settings'          => esc_html__('Settings For POD', 'divi_flash'),
    ...
    public function get_advanced_fields_config() {
        ...
        $advanced_fields['fonts'] = array(
            ...
            'pod_before_after' => array(
                'label'        => esc_html__('Pods', 'divi_flash'),
                'line_height'  => array(
                    'default'  => '1em',
                ),
                'font_size'    => array(
            ...
            'custom_field_before_after' => array(
                'label'           => esc_html__( 'CF', 'divi_flash' ),
                'line_height'     => array(
                    'default' => '1em',
                ),
                'font_size'       => array(
                ...
                'tab_slug'        => 'advanced',
                'toggle_slug'     => 'before_after',
                'sub_toggle'      => 'custom_field',
                'css'             => array(
                    'main'      => "{$this->main_css_element} .cf-before-text, {$this->main_css_element} .cf-after-text",
                    'hover'     => ".df-cpt-outer-wrap:hover {$this->main_css_element} .cf-before-text, .df-cpt-outer-wrap:hover {$this->main_css_element} .cf-after-text",
                    'important' => 'all'
                ),
                'hide_text_align' => true,
            ),
                'reading_time_before_after' => array(
                    'label'           => esc_html__('RT', 'divi_flash'),
                    'line_height'     => array(
                        'default' => '1em',
                    ),
                    'font_size'       => array(
                    ...
                    'tab_slug'        => 'advanced',
                    'toggle_slug'     => 'before_after',
                    'sub_toggle'      => 'reading_time',
                    'css'             => array(
                        'main'      => "{$this->main_css_element} .reading-time-before-text, {$this->main_css_element} .reading-time-after-text",
                        'hover'     => ".df-cpt-outer-wrap:hover {$this->main_css_element} .reading-time-before-text, .df-cpt-outer-wrap:hover {$this->main_css_element} .reading-time-after-text",
                        'important' => 'all'
                    ),
                    'hide_text_align' => true,
                )
    ...
    public function get_fields() {
        ...
        $general = array (
            ...
            'post_type_for_pod'    => array(
                'label'            => esc_html__( 'Post Type', 'et_builder' ),
                'type'             => 'select',
                'option_category'  => 'configuration',
                'options'          => $this->df_pod_cpt_list,
                'description'      => esc_html__( 'Choose posts to display Pods Field.', 'et_builder' ),
                'toggle_slug'      => 'settings',
                'default'          => 'select',
                'show_if'          => array(
                    'type'         => 'pod_fields'
        ...
        if( $this->pod_extension === 'on' && class_exists('Pods') ){
            $general['type']['options']['pod_fields'] = esc_html__('Pods Field', 'divi_flash');
            $general = array_merge( $general, $this->df_pod_fields );
        }
        ...
        $general = array_merge($general, array(
            'pod_before_label'=> array(
                ...
                'default'     => '',
                'show_if'     => array(
                    'type'    => array('pod_fields')
                )
            ),
            'pod_after_label' => array(
                'label'       => esc_html__( 'After Text', 'divi_flash' ),
                'type'        => 'text',
                'toggle_slug' => 'settings',
        ...
        $settings_for_pod = array(
            'pod_url_text' => array(
                'label'       => esc_html__( 'Link Text', 'divi_flash' ),
                'type'        => 'text',
                'toggle_slug' => 'pod_settings',
                'default'     => '',
                'show_if'     => array(
                    'type'    => array('pod_fields')
                )
            ),
            'pod_url_new_window'   => array(
                'label'            => esc_html__( 'Link Target', 'divi_flash' ),
                'type'             => 'select',
                'option_category'  => 'configuration',
                'options'          => array(
        ...
        $icon_settings_dependency = array('author', 'date', 'taxonomy', 'button', 'icon', 'acf_fields', 'pod_fields', 'metabox_fields');
        $icon_settings = array(
            'use_icon'    => array(
    ...
    public function additional_css_styles($render_slug) {
	    ...
	    if(!empty($this->props['image_size'])){
		    ...
		    ET_Builder_Element::set_style($render_slug, array(
			    'selector' => '%%order_class%% .df-cpt-image-wrap img',
        ...
        if($this->props['type'] === 'pod_fields') {
            $this->df_process_range( array(
                'render_slug'       => $render_slug,
                'slug'              => 'pod_image_width',
                'type'              => 'max-width',
                'selector'          => '%%order_class%%.df-item-wrap img.df-pod-image'
            ));
        }
    ...
    public function render( $attrs, $content, $render_slug ) {
        ...
        if('select' !== $this->props['type']) {
            ...
            $type_settings['use_custom_field'] = $this->props['use_custom_field'];
            ...
            // Pods Field
            if('pod_fields' === $this->props['type'] && class_exists('Pods') && 'on' === $this->pod_extension ) {
                $type_settings['post_type_for_pod']  = $this->props['post_type_for_pod'];
                $type_settings['pod_field']          = $this->props['post_type_for_pod'] != '' && $this->props['post_type_for_pod'] != 'select' ?
                    $this->props["pod-".$this->props['post_type_for_pod']] : '';
                $type_settings['pod_before_label']   = $this->props['pod_before_label'] != '' ? $this->props['pod_before_label'] : '';
                $type_settings['pod_after_label']    = $this->props['pod_after_label'] != '' ? $this->props['pod_after_label'] : '';
                $type_settings['pod_url_text']       = $this->props['pod_url_text'] !== '' ? $this->props['pod_url_text'] : '';
                $type_settings['pod_url_new_window'] = $this->props['pod_url_new_window'] !== '' ? $this->props['pod_url_new_window'] : 'off';
                $type_settings['pod_email_text']     = $this->props['pod_email_text'] !== '' ? $this->props['pod_email_text'] : '';
            }
            if('on'===$this->props['use_custom_field']){
