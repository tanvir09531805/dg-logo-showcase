...
const iconPickerVisible = ({ attrs, }) => 'on' === attrs?.imageIcon?.innerContent?.desktop?.value?.useIcon;
const iconPickerInvisible = ({ attrs, }) => 'on' !== attrs?.imageIcon?.innerContent?.desktop?.value?.useIcon;
const imgFullWidth = ({ attrs }) => "on" !== attrs.useImage?.decoration?.fullWidth?.desktop?.value;

const imageAltTextHas = ({ attrs, }) => {
