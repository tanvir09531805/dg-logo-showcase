...
namespace DIFL\Server\Modules\BentoGridItem\Traits;
...
trait StyleDeclaration {
	...
	public static function grid_span_declaration( $params ) {
		$columnSpan = $params['attrValue']['columnSpan'] ?? [];
		$rowSpan = $params['attrValue']['rowSpan'] ?? [];
		...
		if ( ! empty( $columnSpan ) ) {
			$style_declarations->add( 'grid-column', "span {$columnSpan}" );
		}
		if ( ! empty( $rowSpan ) ) {
			$style_declarations->add( 'grid-row', "span {$rowSpan}" );
		}
